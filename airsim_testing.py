...
class AirSimEnv_testing(gym.Env):
    def __init__(self):
        super(AirSimEnv_testing, self).__init__()
        self.mark = 0
        self.success_rate = 0
        self.episode = 0
        ...
        self.reward_net = torch.load('./model/airl/reward/reward_net59.pth', weights_only=False, map_location=torch.device("cuda:0"))
    ...
    def step(self, action):
        ...
        if done:
            ...
            if self.episode_distance > goalY:
                self.success += 1
            self.success_rate = self.success / self.episode
            self.mark = self.success_rate/self.step_len
            print(f'episode distance: {self.episode_distance} maxdistance: {self.max_distance} '
                  f'meandistance: {np.mean(self.distance)} mark: {self.mark}')

            self.episode_distance = 0
            self.step_len = 0
            reward = self.client.getMultirotorState().kinematics_estimated.position.y_val
    ...
    def compute_reward(self, done):
        output = self.client.getMultirotorState().kinematics_estimated.position.y_val
        return output
